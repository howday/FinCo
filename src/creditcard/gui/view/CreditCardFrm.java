 ...
 */
public class CreditCardFrm extends javax.swing.JFrame {
	/****
	...
	CreditCardFrm myframe;
	private Object rowdata[];
	public String acountnumber;

	public CreditCardFrm() {
		myframe = this;
		/**
		...
		model.addColumn("CCNr");
		model.addColumn("Name");
		model.addColumn("City");
		model.addColumn("G/C/S");
...
//		JButton_CompAC.setBounds(240, 20, 192, 33);
		...
		JButton_Addinterest.setText("Generate Report");
	...
	class SymWindow extends java.awt.event.WindowAdapter {
		public void windowClosing(java.awt.event.WindowEvent event) {
			...
			if (object == CreditCardFrm.this)
				CreditCard_windowClosing(event);
		}
	}

	void CreditCard_windowClosing(java.awt.event.WindowEvent event) {
		// to do: code goes here.

		CreditCard_windowClosing_Interaction1(event);
	}

	void CreditCard_windowClosing_Interaction1(java.awt.event.WindowEvent event) {
		try {
	...
	class SymAction implements java.awt.event.ActionListener {
		public void actionPerformed(java.awt.event.ActionEvent event) {
			...
			if (object == JButton_Exit)
				(new CreditCardController(myframe)).JButtonExit_actionPerformed(event);
			else if (object == JButton_PerAC)
				(new CreditCardController(myframe)).JButtonPerAC_actionPerformed(event);
			else if (object == JButton_CompAC)
				(new CreditCardController(myframe)).JButtonCompAC_actionPerformed(event);
			else if (object == JButton_Deposit) {
				if (selection >= 0) {
					(new CreditCardController(myframe)).JButtonDeposit_actionPerformed(event);
				} else {
					JOptionPane.showMessageDialog(getParent(), "Please select an account!!");
				}
			} else if (object == JButton_Withdraw) {
				if (selection >= 0) {
					(new CreditCardController(myframe)).JButtonWithdraw_actionPerformed(event);
				} else {
					JOptionPane.showMessageDialog(getParent(), "Please select an account!!");
				}
			}
